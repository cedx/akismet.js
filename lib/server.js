...
Server.prototype.checkComment=function(req, res) {
  var clt=req.akismet.client;
  var self=this;
  return clt.checkComment(req.akismet.comment).then(
...
Server.prototype.submitHam=function(req, res) {
  var clt=req.akismet.client;
  var self=this;
  return clt.submitHam(req.akismet.comment).then(
...
Server.prototype.submitSpam=function(req, res) {
  var clt=req.akismet.client;
  var self=this;
  return clt.submitSpam(req.akismet.comment).then(
    function() { Server._sendResponse(res, 'Thanks for making the web a better place.'); },
    function(err) { Server._sendResponse(res, '', err); self.emit('error', err); }
  );
};
...
Server._processRequest=function(req, res, next) {
  ...
  var clt=new client.Client(apiKey, client.Blog.fromJSON(req.body), { userAgent: userAgent });

  // Parse comment values.
  var cmt=comment.Comment.fromJSON(req.body);
  if(!cmt.author) cmt.author=new comment.Author();
  if(!cmt.author.ipAddress) cmt.author.ipAddress=req.connection.remoteAddress;
