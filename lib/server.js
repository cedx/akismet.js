...
Server.prototype.checkComment=function(req, res) {
  ...
  client.checkComment(comment, function(err, isSpam) {
    if(err) _sendResponse(res, 'invalid', err);
    else _sendResponse(res, isSpam ? 'true' : 'false');
  });
};
...
Server.prototype.submitHam=function(req, res) {
  ...
  client.submitHam(comment, function(err) {
    if(err) _sendResponse(res, '', err);
    else _sendResponse(res, 'Thanks for making the web a better place.');
  });
};
...
Server.prototype.verifyKey=function(req, res) {
  ...
  client.verifyKey(function(err, isValid) {
    if(err) _sendResponse(res, 'invalid', err);
    else _sendResponse(res, isValid ? 'valid' : 'invalid');
  });
};
...
Server.prototype._registerRoutes=function() {
  ...
  app.all('*', function(req, res) {
    ...
    else if(typeof self.redirectUrl=='string') res.redirect(301, url.resolve(self.redirectUrl, req.url));
    else res.send(404);
  });
};
