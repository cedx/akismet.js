...
Server._processRequest=function(req, res, next) {
  ...
  var client=new Client(apiKey, blog, { userAgent: userAgent });

  // Parse comment values.
  var comment=core.Comment.fromJSON(req.body);
  if(!comment.author) comment.author=new core.Author();
