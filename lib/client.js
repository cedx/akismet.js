...
export class Client {
	...
	constructor(apiKey, blog, options = {}) {
		const {baseUrl = "https://rest.akismet.com/1.1/", isTest = false, userAgent = `Node.js/${version.slice(1)} | Akismet/${pkg.version}`} = options;
		this.apiKey = apiKey;
	...
	async #fetch(endpoint, fields) {
		...
		if (response.headers.has("x-akismet-alert-code")) {
			...
			const error = new Error(response.headers.get("x-akismet-alert-msg") ?? "");
			error.name = code ? `AkismetError #${code}` : "AkismetError";
			throw error;
		}

		if (response.headers.has("x-akismet-debug-help")) {
			const error = new Error(response.headers.get("x-akismet-debug-help") ?? "");
			error.name = "AkismetError";
			throw error;
		}
