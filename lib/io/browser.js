...
export class Client extends BaseClient {
  ...
  constructor(apiKey, blog, options = {}) {
    if (!('userAgent' in options)) options.userAgent = `JavaScript/${process.version.substring(1)} | Akismet/${packageVersion}`;

    super(apiKey, blog, {
      fetch(request) { return fetch(request); },
      newRequest(url) { return new Request(url.href, {mode: 'no-cors'}); }
    });
  }
}
