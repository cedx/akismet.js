...
module.exports = class Server extends EventEmitter {
  ...
  listen(port = -1, address = '') {
    ...
    return new Promise(resolve => this._httpService.listen(port, address, () => {
   ...
   */
  _registerMiddleware() {
    let debug = 'app' in global.app ? global.app.debug : false;
    this._express.use(logger(debug ? 'dev' : 'combined'));
